...
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	// align source with starting pose
	Eigen::Matrix4f init_guess = transform3D(startingPose.rotation.yaw, 
                                             startingPose.rotation.pitch, 
                                             startingPose.rotation.roll, 
                                             startingPose.position.x, 
                                             startingPose.position.y, 
	...
	ndt.setTransformationEpsilon(1e-3); // Setting minimum transformation difference for termination condition.
	ndt.setStepSize(1);                 // Setting maximum step size for More-Thuente line search.
	ndt.setResolution(5);               //Setting Resolution of NDT grid structure (VoxelGridCovariance).
	...
	lidar->Listen([&new_scan, &lastScanTime, &scanCloud](auto data){
		...
		if(new_scan){
			...
			for (auto detection : *scan){
				if((detection.point.x * detection.point.x + 
                    detection.point.y * detection.point.y + 
                    detection.point.z * detection.point.z) > 8.0){ // Don't include points touching ego
